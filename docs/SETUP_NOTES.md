...
## Phase 11: dependency vulnerability decisions

Adding Vitest and testing-library pulled in a new wave of `npm audit` findings. Handled the same way as before — fix what's real and fixable, document what's accepted:

- **`react-router-dom` — fixed, upgraded 6.x → 7.18.1.** This one was a genuine, non-dev-only vulnerability (open redirect + arbitrary constructor injection), and critically, **no patched 6.x version exists** — the fix only landed in the 7.x line. Verified before committing to the jump: our actual usage (`BrowserRouter`, `Routes`, `Route`, `Link`, `NavLink`, `useNavigate`, `useParams`, `Outlet`) is unchanged between v6 and v7 for this basic client-side routing pattern, and a full typecheck + the entire test suite passed without modification after the upgrade. A separate, different vulnerability remains flagged (an RSC-mode CSRF bypass) — this project uses plain client-side routing, not React Server Components / framework mode, so that advisory doesn't apply to this codebase's actual usage.
- **`eslint` chain (`@eslint/config-array`, `@eslint/eslintrc`, `brace-expansion`, `minimatch`) — accepted, same policy as before.** Dev-tooling only (linting), requires `eslint@10` to fully resolve, which is outside this project's deliberately-conservative dependency choices (see above). Not shipped to production.
- **`vite`/`esbuild`/`vite-node`/`@vitest/mocker` chain — accepted, same dev-server-only issue already documented above**, now also surfacing through Vitest's own internal Vite dependency. Same root cause, same reasoning: dev-only, not shipped to production, requires the same `vite@8` jump already deferred.
